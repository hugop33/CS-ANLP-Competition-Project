...
def main():
    ...
    # Get the first most confident label
        'Label' : target1,
        'ID' : np.arange(len(target1)),
    })

    df_target2nd = pd.DataFrame({
    # Get the second most confident label
        'Label' : target2,
        'ID' : np.arange(len(target2)),
    })

    df_target_05 = pd.DataFrame({
    # Get the second most confident label if the difference between the first and second most confident labels is less than 0.05, otherwise the first one
        'Label' : label_05,
        'ID' : np.arange(len(label_05)),
    })

    df_target_1 = pd.DataFrame({
    # Get the second most confident label if the difference between the first and second most confident labels is less than 0.1, otherwise the first one
        'Label' : label_1,
        'ID' : np.arange(len(label_1)),
    })

    df_target_05_1 = pd.DataFrame({
    # Get the second most confident label if the difference between the first and second most confident labels is between 0.05 and 0.1, otherwise the first one
        'Label' : label_05_1,
